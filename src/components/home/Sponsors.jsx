const Sponsors = () => {
  return (
    <div className="relative bg-witl-purple-300 mt-[50%] w-[100%]">
      <div className="text-witl-purple-200 pl-[10%] flex flex-col items-start leading-tight">
        <div className="text-[8rem] mt-[5%]">OUR</div>
        <div className="text-10xl">&nbsp;&nbsp;SPONSORS</div>
      </div>
      <div className="absolute top-[15%] right-[0%] bg-witl-purple-400 text-witl-white-100 text-[4rem] w-[55%]">
        <div className="pl-[3%]">BLUEPRINT TEST PREP</div>
      </div>
      <div className="flex flex-col bg-witl-purple-300 text-3xl text-white font-bold">
        <div className="w-1/2 bg-witl-purple-400 mb-6 mt-4 font-libre-caslon-text py-2 text-right pr-5">
          BEST BEST KRIEGER
        </div>
        <div className="flex justify-end">
          <div className="w-1/3 bg-witl-purple-400 font-libre-caslon-text py-2 pl-12 text-left">
            HQR INC.
          </div>
        </div>
        <div className="border-witl-purple-400 border-b-8 ">
          <div className="text-witl-purple-400 text-sm mb-1 font-Junge">
            Events
          </div>
        </div>
      </div>
    </div>
  );
};

export default Sponsors;
